...
After any production deploy, verify the service is alive: `curl -sS -o /dev/null -w "%{http_code}\n" https://identity.smol.la/health` should return `200`. The same probe against `https://staging-identity.smol.la/health` covers staging.
